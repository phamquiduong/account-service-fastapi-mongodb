...
async def login_for_access_token(
    ...
    user = await authenticate_user(user_service, email=form_data.username, password=form_data.password)
...
async def login_with_email_password(user_service: UserServiceDep, request: LoginRequest) -> TokenResponse:
    user = await authenticate_user(user_service, email=request.email, password=request.password)
    access_token_data = TokenData.access(user=user)

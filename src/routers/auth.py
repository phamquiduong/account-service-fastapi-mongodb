...
async def login_for_access_token(
    ...
    access_token_data = TokenData.access(user=user, token_version=token_version.version)
...
async def login_with_email_password(user_service: UserServiceDep, request: LoginRequest) -> TokenResponse:
    ...
    token_version = await user_service.get_token_version(user_id=user.id)
    access_token_data = TokenData.access(user=user, token_version=token_version.version)
    refresh_token_data = TokenData.refresh(user=user, token_version=token_version.version)
